...
export function Toolbox({ buttonSectionId, title }: ToolboxProps) {
  ...
  return (
    <PanelSection>
      <PanelSection.Header className="flex items-center justify-between">
        ...
        {CustomConfigComponent && (
          <div className="ml-auto mr-2">
            <Icons.Settings
              className={`h-4 w-4 ${showConfig ? 'text-highlight' : 'text-primary-active'}`}
              onClick={e => {
                e.stopPropagation();
      ...
      <PanelSection.Content className="bg-background flex-shrink-0 border-none">
        ...
        {toolboxSections.map(section => {
          ...
          return (
            <div
              key={sectionId}
              className="bg-background flex flex-wrap gap-2 py-2 px-1"
            >
              {buttons.map(tool => {
